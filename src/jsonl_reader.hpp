...
[[nodiscard]] std::unordered_set<std::string> load_event_ids(const std::string& path);

/// Load all events from a JSONL file (full parse).
[[nodiscard]] std::vector<Event> load_events(const std::string& path);

/// Get min_depth, max_depth, root_event_id, and branching factor in a single pass.
...
[[nodiscard]] std::map<int64_t, DepthBucket> get_depth_profile(const std::string& path);
